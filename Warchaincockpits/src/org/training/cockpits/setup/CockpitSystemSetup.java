...
import org.training.cockpits.constants.WarchainCockpitsConstants;
 ...
 */
@SystemSetup(extension = WarchainCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(WarchainCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(WarchainCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						WarchainCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/Warchaincockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
