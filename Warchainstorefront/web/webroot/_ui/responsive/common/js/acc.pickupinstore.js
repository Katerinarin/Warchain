...
ACC.pickupinstore = {
	...
	createListItemHtml: function (data,id){
		...
		item+='<label for="pickup-entry-'+id+'" class="js-select-store-label" data-store-lat="'+data.storeLatitude+'" data-store-long="'+data.storeLongitude+'" data-warehouse-lat="'+data.warehouseLat+'" data-wareHouse-long="'+data.warehouseLong+'">';
		item+='<span class="pickup-store-info">';
		item+='<span class="pickup-store-list-entry-name">'+data.userLocation.lat+'    '+data.displayName+'</span>';
		...
		item+='<span class="pickup-store-list-entry-city">'+data.userLocation+'</span>';
		item+='</span>';
	...
	refreshPickupInStoreColumn: function (data){
		...
		$(document).ready(function(){
			...
			var test = $("#colorbox .js-select-store-label")[0];
			$(test).click();
			$(firstInput).click();
		});
	...
	pickupStorePager:function()
	{
		...
		$(document).on("click",".js-select-store-label",function(e){
			...
			if (navigator.geolocation) { //проверка включена ли у пользователя геолокация
				navigator.geolocation.getCurrentPosition(function(position) { //получение текущей позиции
					
					if (position.coords.latitude != "" && position.coords.longitude != "") { //пользователя
						supplyChain.push([position.coords.longitude, position.coords.latitude]);
					}
					if (selectedStore.data("store-lat") != "" && selectedStore.data("store-long") != "") { //магазина
						supplyChain.push([selectedStore.data("store-long"), selectedStore.data("store-lat")]);
					}
					if (selectedStore.data("warehouse-lat") != "" && selectedStore.data("warehouse-long") != "") { //склада
